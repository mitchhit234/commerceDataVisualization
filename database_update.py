...
def get_max_col(cur,col_name):
  statement = "SELECT MAX(" + col_name + ") FROM TRANSACTIONS"
...
conn = dc.create_database(database_name)
cursor = conn.cursor()
current_num = get_max_col(cursor,"num") + 1
last_date = get_max_col(cursor,"date")

#Query ran on the GMail Inbox
#Depth indicates how many entries max
#will be returned
depth_of_search = 100
alert_address = "CommerceBankAlerts@commercebank.com"
L = service.users().messages().list(userId='me', maxResults=depth_of_search, q='from:commercebankalerts@commercebank.com').execute()
...
for i in valid_values:
  ...
  statement = dc.generate_insert_sql(table_name,i)
  print(statement)
  try:
    cursor.execute(statement)
  except dc.Error as e:
    dc.traceback.print_exc()